...
export async function secureDocumentUpload(formPayload: FormData): Promise<UploadResponse> {
  try {
    ...
    if (!verifiedMimeType) {
      return {
        success: false,
        errorMessage: 'Invalid file format detected. Only PDF, PNG, and JPEG are allowed.',
      };
    }
    ...
    if (secureS3Client && isAwsConfigured) {
      ...
      console.warn(
        'AWS S3 environment variables not configured. Falling back to local mock upload.'
      );
